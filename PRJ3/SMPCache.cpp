    ...
    , allocDirty("%s:allocDirty", name)
{
    ...
    /* add code for missing count */
    confCacheBlockNumber = SescConf->getInt(section, "size")/64;
    /* add code for missing count */
...
/* add code for update fully asso cache */
void SMPCache::updateLRUInfo(PAddr tag)
{
    std::vector<PAddr>::iterator it;
    it = std::find(fullAssoCache.begin(), fullAssoCache.end(), tag);
    if (it == fullAssoCache.end()){ // tag not in the simulated cache
        if (fullAssoCache.size() == confCacheBlockNumber)
            fullAssoCache.erase(fullAssoCache.begin());
    } else {
        fullAssoCache.erase(it);
    }
    fullAssoCache.push_back(tag);
}

void SMPCache::updateWriteMissInfo(PAddr addr)
{
    PAddr tag = calcTag(addr); // block reference
    if (accessedBlockRecord.count(tag) == 0) {
        accessedBlockRecord.insert(std::pair<PAddr, int>(tag, 1));
        writeCompMiss.inc();
    } else {
        writeReplMiss.inc();
    }
...
void SMPCache::updateReadMissInfo(PAddr addr)
{
    PAddr tag = calcTag(addr); // block reference
    if (accessedBlockRecord.count(tag) == 0) {
        accessedBlockRecord.insert(std::pair<PAddr, int>(tag, 1));
...
void SMPCache::updateHitInfo(PAddr addr)
{
    PAddr tag = calcTag(addr); // block reference
    updateLRUInfo(tag);
}
...
void SMPCache::doRead(MemRequest *mreq)
{
    ...
    /* add code for classify the miss and cohe miss */
    if (InvalidTags.find(calcTag(addr)) != InvalidTags.end()){
        readCoheMiss.inc();
    } else {
        updateReadMissInfo(addr);
    }
    /* add code for classify the miss and cohe miss */
...
void SMPCache::realInvalidate(PAddr addr, ushort size, bool writeBack)
{
    while(size) {
        ...
        if (l) {
            ...
            /* add code to get the invalidate line */
            InvalidTags.insert(l->getTag());
            /* add code to get the invalidate line */
            l->invalidate();
        }
                                       ...
                                       bool canDestroyCB)
{
    ...
    if(!l->isValid()) {
        ...
        /* add code to remove invalid line and update LRU */ 
        InvalidTags.erase(l->getTag());
        l->setTag(cache->calcTag(addr));
        updateLRUInfo(l->getTag());
        /* add code to remove invalid line and update LRU */ 
