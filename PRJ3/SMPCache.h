...
protected:
    ...
    /* project 3 type miss -- my code */
    uint32_t confCacheBlockNumber; // record the cache size
    std::map<PAddr, int> accessedBlockRecord; // record all block accessed
    std::vector<PAddr> fullAssoCache; // simulate fully associative LRU cache
    std::set<uint> InvalidTags; // record invalid tags by multi-core
    void updateLRUInfo(PAddr tag);
    void updateReadMissInfo(PAddr addr);
    void updateWriteMissInfo(PAddr addr);
    void updateHitInfo(PAddr addr);
    /* project 3 type miss -- my code */
