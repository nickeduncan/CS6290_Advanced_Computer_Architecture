...
PredType  BPNotTaken::predict(const Instruction * inst, InstID oracleID, bool doUpdate)
{
    bpredEnergy->inc();
    /* add code */
    HistoryType iID = inst->calcNextInstID();
    if (InstructionID.count(iID) == 0){
        missPredictCount[InstructionCounter] = 0;
        correctPredictCount[InstructionCounter] = 0;
        InstructionID.insert(std::pair<HistoryType, int>(iID, InstructionCounter));
        InstructionCounter++;
    }

    if (iID == oracleID) {
        correctPredictCount[InstructionID.at(iID)]++;
        return CorrectPrediction;
    } else {
        missPredictCount[InstructionID.at(iID)]++;
        return MissPrediction;
    }
    /* add code */
}
...
BPNotTaken::~BPNotTaken(){
    ...
    std::map<HistoryType, int>::iterator itr;
    for(itr = InstructionID.begin(); itr != InstructionID.end(); itr++){
        int instructionPosition = itr->second;
        int instructionRunCount = missPredictCount[instructionPosition] + correctPredictCount[instructionPosition];
        if (instructionRunCount >= 1000){
            GroupIns[3] ++;
            GroupMiss[3] += missPredictCount[instructionPosition];
            GroupCorrect[3] += correctPredictCount[instructionPosition];
        } else if (instructionRunCount >= 100){
            GroupIns[2] ++;
            GroupMiss[2] += missPredictCount[instructionPosition];
            GroupCorrect[2] += correctPredictCount[instructionPosition];
        } else if (instructionRunCount >= 10){
            GroupIns[1] ++;
            GroupMiss[1] += missPredictCount[instructionPosition];
            GroupCorrect[1] += correctPredictCount[instructionPosition];
        } else {
            GroupIns[0] ++;
            GroupMiss[0] += missPredictCount[instructionPosition];
            GroupCorrect[0] += correctPredictCount[instructionPosition];
        }
    }
    std::cout << "==========================NotTaken--Predictor==========================" << std::endl;
    std::cout << "1-9 Total: " << GroupMiss[0] + GroupCorrect[0] <<"   Predict Correct: " << GroupCorrect[0] << "  Predict Wrong:" << GroupMiss[0] << " Ins Number: " << GroupIns[0] << endl;
    std::cout << "10-99 Total: " << GroupMiss[1] + GroupCorrect[1] <<"   Predict Correct: " << GroupCorrect[1] << "  Predict Wrong:" << GroupMiss[1] << "   Ins Number: " << GroupIns[1] << endl;
    std::cout << "100-999 Total: " << GroupMiss[2] + GroupCorrect[2] <<"   Predict Correct: " << GroupCorrect[2] << "  Predict Wrong:" << GroupMiss[2] << " Ins Number: " << GroupIns[2] << endl;
    std::cout << "1000+ Total: " << GroupMiss[3] + GroupCorrect[3] <<"   Predict Correct: " << GroupCorrect[3] << "  Predict Wrong:" << GroupMiss[3] << "   Ins Number: " << GroupIns[3] << endl;
...
BPHybrid::~BPHybrid()
{
    ...
    std::cout << "===========================Hybrid--Predictor===========================" << std::endl;
    std::cout << "1-9 Total: " << GroupMiss[0] + GroupCorrect[0] <<"   Predict Correct: " << GroupCorrect[0] << "  Predict Wrong:" << GroupMiss[0] << " Ins Number: " << GroupIns[0] << endl;
    std::cout << "10-99 Total: " << GroupMiss[1] + GroupCorrect[1] <<"   Predict Correct: " << GroupCorrect[1] << "  Predict Wrong:" << GroupMiss[1] << "   Ins Number: " << GroupIns[1] << endl;
    std::cout << "100-999 Total: " << GroupMiss[2] + GroupCorrect[2] <<"   Predict Correct: " << GroupCorrect[2] << "  Predict Wrong:" << GroupMiss[2] << " Ins Number: " << GroupIns[2] << endl;
    std::cout << "1000+ Total: " << GroupMiss[3] + GroupCorrect[3] <<"   Predict Correct: " << GroupCorrect[3] << "  Predict Wrong:" << GroupMiss[3] << "   Ins Number: " << GroupIns[3] << endl;
...
PredType BPHybrid::predict(const Instruction *inst, InstID oracleID, bool doUpdate)
{
    ...
    if( inst->isBranchTaken() ){
        if (btb.predict(inst, oracleID, doUpdate) == MissPrediction){
            missPredictCount[InstructionID.at(iID)]++;
            return MissPrediction;
        } else {
            correctPredictCount[InstructionID.at(iID)]++;
